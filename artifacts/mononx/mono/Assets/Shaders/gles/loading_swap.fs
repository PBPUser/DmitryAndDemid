...
void main(){
	...
	float currentIndex = (1.0-(fragTexCoord.y+1.0)) * 8.0;
	float iD = mod(currentIndex, 1.);
	int cI1 = int(currentIndex);
	int cI2 = int(currentIndex+1.0)%8;
	float jv1 = clamp(p[cI1][0],-2.0,2.0);
	float jv2 = clamp(p[cI2][0],-2.0,2.0);
	...
	jv1 = clamp(p[cI1][0],-1.0,1.0);
	jv2 = clamp(p[cI2][0],-1.0,1.0);
	pos.y = 1.0-pos.y;
	sx = jv1 + (jv2-jv1)*iD;
	if(pow(t, sx) * (t * 2.) > 1.0-fragTexCoord.x)
		transparency = 1.0;
	if(time > 1.0)
