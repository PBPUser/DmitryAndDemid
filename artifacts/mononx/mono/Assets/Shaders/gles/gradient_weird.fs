...
float getSmoothColor(vec2 uv, float border_width){
    ...
    for(float y = 0.0; y < border_width; y++)
        color += texture(texture0, uv+vec2(dx*(x-border_width/2.0), dy*(y-border_width/2.0)))[3] / max(1., abs((x-border_width)*(y-border_width)));
...
void main(){
    ...
    _fragColorOut = mix(color_grad, border_color, delta_value);
}
