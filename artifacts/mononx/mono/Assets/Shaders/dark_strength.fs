...
void main(){ 
  ...
  float maxP = clamp(2-time * 3, 0, 1);
  float minP = clamp(1-time * 3, 0, 1);
  float maxPd = clamp(2-timed * 3, 0, 1);
  float minPd = clamp(1-timed * 3, 0, 1);
  if((mask & 0x1) == 0x1){
  ...
  float tm = clamp(5-abs(time * 10 - 5),0,1);
  float tm2 = clamp(5-abs(time * 10 - 3),0,1);
  float cl = renderClouds(ftc * res, timed * 100, tm*c*4-1.5);
  float cl2 = renderClouds(ftc * res, time * 100, tm*c1*8-7);
  float cl3 = renderClouds(ftc * res, time * 100, tm*c2*8-7);
  gl_FragColor = mix(gl_FragColor, vec4(0.64, 0.16, 0.16, 1), cl3 * .5);
  gl_FragColor = mix(gl_FragColor, vec4(0,1,0,1), cl2 * .5);
  gl_FragColor = mix(gl_FragColor, vec4(0,0,0,1), cl * (min(timed, 0.2) / 0.2));
}
