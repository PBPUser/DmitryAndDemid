...
void main(){
    ...
    vec2 r = zoom((fragTexCoord + vec2(0, -.7)) * vec2(1, tSize2.y/tSize2.x * tSize.x/tSize.y * -1), 4, vec2(0.5));
    float angle = 0.4;
    ...
    if(abs(r.y) < 4){
        ...
        gl_FragColor = mix(gl_FragColor, vec4(j.rgb, 1), j[3] * .3 * clamp(2 - abs(time * 4 - 2), 0, 1));
    }
