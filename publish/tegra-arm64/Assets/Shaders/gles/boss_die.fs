...
vec4 bloomLeavesColor(vec2 uv){
  ...
  for(float n = 0.0; n < 9.0; n++){
    ...
    for(float i = 0.0; i < 9.0; i++)
      hs+=c1leavesColor(uv + texelSize * bloomSpread * vec2(n-4.0,i-4.0));
     sum += hs / 9.0;  
    }
...
float pseudoRandom(float j){
  return mod(sin(tan((cos(position.x) + position.y) * j)) + 1000.0, 1.0);
}
...
void main(){
  ...
  ftc.y = 1.0-ftc.y;
  ftc = ftc * res;
  float t = clamp(time * 3.0 - 1.0, 0.0, 1.0);
  ...
  float overlayOpacity = 1.0 - (max(0.0, time-0.75) * 4.0);
  for(int i = 0; i < arr.length(); i++){
    tTime = max(time - arr[i][1][1],0.0);
    ...
    _fragColorOut = mix(_fragColorOut, vec4(fColor.rgb,1), fColor[3] * overlayOpacity);
  }
}
