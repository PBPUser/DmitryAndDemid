...
void main()
{
    ...
    _fragColorOut = texture(texture0, zoom(fragTexCoord, vec2(.5), (1.0-pow(1.0-distance(fragTexCoord,vec2(.5, 1.5)), 4.0))) * .25 + .75);
    _fragColorOut = mix(_fragColorOut, vec4(.3, .7, 1.0,1.0), pow(distance((fragTexCoord + vec2(0.0,-1.0)) * res, vec2(.5, .5) * res) / 448.0, 3.0));
    ...
    _fragColorOut = vec4(_fragColorOut.rgb * (1.0-c[3]) + c.rgb * c[3] * 1.0,1.0);
	_fragColorOut = vec4(_fragColorOut.rgb * (1.0-c2[3]) + c2.rgb * c2[3] * 1.0,1.0);
}
