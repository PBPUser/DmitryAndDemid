...
void main()
{
    ...
    vec3 bg;
    bg.r = texture2D(texture0, fragTexCoord + offset * 1.08).r;
    bg.g = texture2D(texture0, fragTexCoord + offset).g;
    bg.b = texture2D(texture0, fragTexCoord + offset * 0.92).b;
    ...
    gl_FragColor = vec4(mix(vec3(0.0), col, insideMask), mix(shadowA, glassA, insideMask));
}
