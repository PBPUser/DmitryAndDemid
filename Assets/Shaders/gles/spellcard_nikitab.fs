...
uniform float time;
uniform vec2 pos;
uniform sampler2D pizza;   // tiled overlay layer (bound by GameBox to pizza.png)
...
void main()
{
	...
	uv.x += sin(uv.y * 40.0 * sin(time * 40.0) + time * 40.0) * .01;
	uv.y += sin(uv.x * 40.0 * cos(time * 17.0) + time * 40.0) * .01;
	vec2 quv = uv + vec2(sin(time * 200.0), cos(time* 200.0)) * .01;
	vec2 ruv = rotate2D(quv, pos / res, sin(time * 100.0) * .1);
    vec4 texelColor = texture(texture0,wrapCoordinates(ruv));
	...
	// First layer: the nikitos distortion with cloud colour ADDED to it.
	_fragColorOut = vec4(_fragColorOut.rgb + c.rgb * c[3] * 2.0,1.0);

	// Tiled pizza layer displayed above, slowly drifting, blended in faintly (~0.08).
	vec2 puv = fract(fragTexCoord * vec2(3.0, 3.5) + vec2(time * 5.0, time * 3.0));
	vec4 pz = texture(pizza, puv);
	_fragColorOut.rgb += pz.rgb * pz.a * 0.08;
	_fragColorOut.a = 1.0;
}
