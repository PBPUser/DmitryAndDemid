...
float bloom(vec2 uv){
    ...
    float sum = 0.0, hsum = 0.0;
    for(float n = 0.0; n < 9.0; n++){
        hsum = 0.0;
        for(float i = 0.0; i < 9.0; i++)
            hsum += tex(uv + texelSize * bloomSpread * vec2(n-4.0,i-4.0));
        sum += hsum / 9.0; 
    }
    return sum / 9.0 * bloomIntensity;
}
...
void main(){
    ...
    p = rotate(p, vec2(.5), sin(cos(realTime) * 2.0) * 0.3, cos(realTime * 2.0 + sin(ftc.x * 20.0 * cos(realTime)) * 0.1) * 0.3, realTime*2.0);
    p = zoom(p, vec2(.5), 1.0 + sin(realTime * 1.25) * 0.15);
    _fragColorOut = mix(_fragColorOut, color, bloom(p));
}
