...
void main(){
  ...
  _fragColorOut = mix(_fragColorOut, vec4(c1.rgb,1), c1[3]);
  ...
  for(float i = 0.0; i < 16.0; i++){
    ...
    if(tTime > 0.0){
      ...
      if(mod(fPos,max(fSize.x, fSize.y) + 1.0) != fPos)
        continue;
      fPos.y = 1.0 - fPos.y;
      fAngle = pseudoRandom(2.4+i) + tTime * (pseudoRandom(5.2+i) - .5) * 10.0;
      fColor = leavesColor(fragTexCoord, fSize, fPos, fAngle, texel2);
      _fragColorOut = mix(_fragColorOut, vec4(fColor.rgb,1), fColor[3] * overlayOpacity * fOpacity * 0.4);
    }
  }
}
