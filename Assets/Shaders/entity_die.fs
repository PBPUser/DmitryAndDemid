...
const float bloomSpread = 10;
const float bloomIntensity = 20;
...
float bloomLeavesColor(vec2 uv){
...
vec4 leavesColor(vec2 uv, vec2 size, vec2 center){
  vec2 uvi = uv;
  vec2 pivot = (center + size / 2);
  uvi = uvi - pivot;
  vec2 p = (uvi / size) + pivot;
  vec4 tc = texture(textureLeaves, p);
  return vec4(vec3(1), bloomLeavesColor(p));
}
...
void main(){
  ...
  for(int i = 0; i < arr.length(); i++)
  {
    ...
    arr[i][3] = vec2(pseudoRandom(9.1+i), pseudoRandom(i-3.7));
  }
  ftc.y = 1-ftc.y;
  ftc = ftc * res;
  ...
  for(int i = 0; i < arr.length(); i++){
    ...
    fColor = leavesColor(fragTexCoord, fSize, fPos);
